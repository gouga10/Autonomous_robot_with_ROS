...
const double TICKS_PER_REVOLUTION = 227; // For reference purposes.
const double WHEEL_RADIUS = 0.0305; // Wheel radius in meters
const double WHEEL_BASE = 0.15; // Center of left tire to center of right tire
const double TICKS_PER_METER_right = 1093.33; 
const double TICKS_PER_METER_left = 1110;
...
void Calc_Right(const std_msgs::Int16& rightCount) {
  ...
  if(rightCount.data != 0 && lastCountR != 0) {
    ...
    if (distanceRight > 10000) {
      distanceRight = (0 - (65535 - distanceRight))/TICKS_PER_METER_right;
    }
